...
- # Definizione formale di interprete per un linguaggio: ^68f4bc
	...
	- ## Spiegazione:
		- l'interprete $I_{L}^{L_{O}}$ è scritto in $L_{O}$ e prende come input programmi scritti in $L$ e un qualsiasi tipo di dati $D$ che vengono "usati" dal programma $Prog$ il cui output finale è un dato $D$ 
		- Per far si che l'interprete funzioni bene serve ottenere lo stesso risultato che si otterrebbe se non si usasse l'interprete per quel programma 